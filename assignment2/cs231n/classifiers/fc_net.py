...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1):
            f_out, forward_cache[str(i+1)] = affine_forward(X, self.params['w'+str(i+1)], self.params['b' + str(i+1)])
            if self.use_batchnorm:
                bn_out, bn_cache[str(i+1)] = batchnorm_forward(f_out, self.params['gamma'+str(i+1)], self.params['beta' + str(i+1)])
                relu_out, relu_cache[str(i+1)] = relu_forward(bn_out)

            else:
                relu_out, relu_cache[str(i+1)] = relu_forward(fn_out)

            if self.use_dropout:
                relu_out, dropout_cache[str(i+1)] = dropout_forward(relu_out, self.dropout_param)
            X = relu_out.copy()

        scores, final_cache = affine_forward(X, self.params['w' + str(num_layers)], self.params['b' + str(num_layers)])
        ...
        loss += 0.5*self.reg*(np.sum(np.square(self.params['w' + str(num_layers)])))
        ...
        for i in range(self.num_layers-1,0,-1):
            ...
            if self.use_batchnorm:
                ...
                dx_last, dw_last, db_last = affine_backward(dbatchnorm, forward_cache[str(i)])
