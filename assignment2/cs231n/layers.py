...
def affine_forward(x, w, b):
    ...
    ###########################################################################
    num_X = x.shape[0]
    reshaped_input = np.reshape(x,[num_X,-1])
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        mu = np.mean(x, axis=0)
        var = np.var(x , axis=0)
        normalised_data = (x - mu)/np.sqrt(var + eps)
        out = gamma*normalised_data + beta
        ...
        cache = {
            ...
            'inverse_var': 1.0/np.sqrt(var + eps)
        }
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
...
def batchnorm_backward(dout, cache):
    ...
    normalised_data = cache.get('normalised_data')
    ###########################################################################
    # TODO: Implement the backward pass for batch normalization. Store the    #
    # results in the dx, dgamma, and dbeta variables.                         #
    ###########################################################################
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(dout * normalised_data, axis=0)
    dxcap = gamma*dout
    dvar = np.sum(dxcap*cache.get('x_mean'), axis=0) * (cache.get('inverse_var')**3)
    dmu = np.sum(-dxcap*cache.get['inverse_var'], axis=0) + dvar*(-2.0 * cache.get('x_mean'))
    dx = np.sum(dxcap*cache.get('inverse_var'), axis=0) + dmu/dout.shape[0] + dvar*2*cache.get('x_mean')/dout.shape[0]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    output_h = np.int((H + 2*pad - HH)/stride) + 1
    output_w = np.int((W + 2*pad - WW)/stride) + 1
        ...
        for f in range(F):
            for j in range(H_out):
                for i in range(W_out):
                    dw[f,...] += dout[n,f,j,i]*padded_x[n,:,j*stride:j*stride+HH,i*stride:i*stride+WW]
    
    for n in range(N):
        for f in range(F):
            for j in range(H_out):
                for i in range(W_out):
                    dx_temp[n,:,j*stride:j*stride+HH,i*stride:i*stride+WW] += dout[n,f,j,i]*w[f,...]
...
def max_pool_forward_naive(x, pool_param):
    ...
    out_w = np.int((H-pool_width)/stride + 1)
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    out, cache = batchnorm_forward(x_flat, gama, beta, bn_param)
