...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
            ...
            grads['b'], grads['Wx'], grads['Wh'] = db, dWx, dWh
        else:
    ...
    def sample(self, features, max_length=30):
        ...
        if self.cell_type == 'lstm':
            current_cell_state = np.zeros_like(current_hidden_state)

        word_embed, _ = word_embedding_forward(self._start, W_embed)

        for i in range(max_length):
            if self.cell_type == 'rnn':
                current_hidden_state, _ = rnn_step_forward(word_embed, current_hidden_state, Wx, Wh, b)
            else:
                current_hidden_state, current_cell_state, _ = lstm_step_forward(word_embed, current_hidden_state, current_cell_state, Wx, Wh, b)

            current_scores, _ = affine_forward(current_hidden_state, W_vocab, b_vocab)

            captions[:, i] = np.argmax(current_scores, axis=1)
